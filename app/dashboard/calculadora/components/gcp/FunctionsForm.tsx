...
import { calcGCPFunctions } from '../../gcp-pricing';
import type { GCPFunctionsConfig } from '../../gcp-types';

interface Props { onAdd: (name: string, data: GCPFunctionsConfig) => void; }
...
export default function FunctionsForm({ onAdd }: Props) {
  const [name, setName] = useState('Cloud Function');
  const [invocations, setInvocations] = useState(5);
  const [duration, setDuration] = useState(200);
  const [memory, setMemory] = useState(256);

  const cfg: GCPFunctionsConfig = {
    invocationsMillions: invocations,
    avgDurationMs: duration,
    memorySizeMB: memory,
  };
  const monthly = calcGCPFunctions(cfg);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          <label className="text-xs font-semibold text-slate-600 uppercase tracking-wide">Invocaciones (millones/mes)</label>
          <input
            type="number" min={0} step={0.1} value={invocations}
            onChange={e => setInvocations(Math.max(0, Number(e.target.value)))}
            className="mt-1 w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-300"
          />
        </div>
      ...
      <p className="text-xs text-slate-400">Incluye 2M de invocaciones y 400,000 GB-s gratis por mes (2nd gen).</p>
