...
import { GCP_DISK_TYPES, calcGCPDisk } from '../../gcp-pricing';
import type { GCPDiskConfig } from '../../gcp-types';

interface Props { onAdd: (name: string, data: GCPDiskConfig) => void; }
...
export default function DiskForm({ onAdd }: Props) {
  const [name,   setName]   = useState('Persistent Disk');
  const [type,   setType]   = useState<GCPDiskConfig['diskType']>('pd_balanced');
  const [sizeGB, setSizeGB] = useState(128);
  const [qty,    setQty]    = useState(1);
  const [iops,   setIops]   = useState(3000);

  const cfg: GCPDiskConfig = { diskType: type, sizeGB, quantity: qty, provisionedIops: iops };
  const monthly = calcGCPDisk(cfg);
  const diskInfo = GCP_DISK_TYPES[type];
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          ...
          <select
            value={type} onChange={e => setType(e.target.value as GCPDiskConfig['diskType'])}
            className="mt-1 w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-300"
          >
            {Object.entries(GCP_DISK_TYPES).map(([k, v]) => (
        ...
        {type === 'pd_extreme' && (
          <div className="sm:col-span-2">
            ...
            <input
              type="number" min={100} max={120000} value={iops}
