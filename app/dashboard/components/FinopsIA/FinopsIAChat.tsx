...
export default function FinopsIAChat({ messages, isLoading, error, onSend, onClose, onClear }: Props) {
  ...
  return (
    <div className="fixed bottom-24 right-4 sm:right-6 z-50 flex flex-col w-[calc(100vw-2rem)] max-w-sm sm:max-w-md h-[70vh] max-h-[600px] bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-blue-700 to-blue-600 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-blue-200 text-xs">Arquitecto AWS Enterprise</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <form onSubmit={handleSubmit} className="px-3 py-3 border-t border-slate-700 bg-slate-900 shrink-0">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            onKeyDown={handleKey}
            placeholder="Pregúntame sobre AWS y FinOps..."
            rows={1}
            disabled={isLoading}
